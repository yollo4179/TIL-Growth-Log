...
Vue 컴포넌트는 이미 반응형 상태를 관리하고 있다. <br>
상태 === 데이터 <br>


1. 상태: 앱구동에 필요한 기본 데이터 
2. 뷰: 상태를 선언적으로 매핑하여 시각화
3. 기능: 뷰에서 사용자 입력에 대해 반응적으로 상태를 변경할수 있게 정의된 동작. 
![alt text](image-4.png)
<strong> `단방향 데이터 흐름`의 간단한 표현</strong>


# `여러 컴포넌트`가 `상태를 공유`할 떄, 
1. 여러 뷰가 동일한 상태에 종속되는 경우.
2. 서로 다른 뷰의 기능이 동잃한 상태를 변경시켜야하는 경우.
...
- 마찬가지로 관리의 패턴이 깨지기 귑고 유지 관리할 수 없는 코드가 됨 
